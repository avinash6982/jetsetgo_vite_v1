...
const Hero = ({ isHomeScreen }) => {
  ...
  const handleSearch = () => {
    if (
      ...
      navigate(`/explore?${createUrlParamsFromObject(object)}`);
    }
  };
  ...
  return (
    <>
      <header
        className={`flex flex-col relative w-full px-7 py-4 ${
          isHomeScreen ? "h-[529px] items-center" : ""
        }`}
      >
        {isHomeScreen && (
          <div className="flex justify-center items-center">
            <h1 className="font-extrabold text-5xl sm:text-7xl md:text-8xl text-center leading-[55px] sm:leading-[70px] md:leading-[90px] text-gradient">
              Dont think twice, <br /> Just book once !
            </h1>
          </div>
        )}

        <div
          className={`flex w-full max-w-[1024px] lg:h-[65px] lg:flex-row items-center flex-col  shadowCard relative ${
            isHomeScreen ? "mt-20" : ""
          }`}
        >
